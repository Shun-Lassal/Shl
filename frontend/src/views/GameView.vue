  ...
  <div class="min-h-[calc(100vh-4rem)]">
    <div class="mx-auto max-w-7xl px-4 py-6 space-y-6">
      <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <UiButton variant="secondary" @click="returnToLobby">Retour</UiButton>
      </div>

      <UiAlert v-if="gameStore.error" variant="danger">
        {{ gameStore.error }}
      </UiAlert>

      <UiCard v-if="!gameStore.currentGame" class="text-center py-16" padding="lg">
      ...
      <div v-else class="space-y-4 sm:space-y-6">
        ...
        <div v-if="gameStore.currentGame.phase === 'GAME_OVER'" class="space-y-4">
          <UiCard class="text-center" padding="lg">
            ...
            <div class="mt-8 flex justify-center gap-3">
              <UiButton variant="secondary" @click="returnToLobby">Retour aux lobbies</UiButton>
            </div>
          </UiCard>
        </div>
        ...
        <div v-else-if="gameStore.currentGame.phase === 'REWARD'" class="space-y-4">
          <UiCard padding="md">
            <div class="flex items-center justify-between">
              ...
              <UiButton
                :variant="canConfirmReward ? 'primary' : 'secondary'"
          ...
          <UiCard v-if="gameStore.rewardOptions.length === 0" class="text-center py-10" padding="lg">
            <div class="mx-auto flex w-fit items-center gap-3 text-slate-300">
              <UiSpinner />
              <span class="text-sm">Chargement des récompenses…</span>
            </div>
          </UiCard>
        ...
        <div class="flex gap-3 overflow-x-auto pb-1 md:grid md:grid-cols-2 md:gap-4 lg:grid-cols-3 xl:grid-cols-4 md:overflow-visible">
          <GameEnemyCard
            ...
            :selectable="targetMode === 'ENEMIES' && needsTargets"
            :show-selection-hint="!!selectedCard && needsTargets && targetMode === 'ENEMIES'"
            :target-label="enemy.intent.targets?.length ? formatTargetNames(enemy.intent.targets) : null"
            @toggle="toggleEnemyTarget(enemy.id)"
          />
        </div>
        ...
        <div class="flex gap-3 overflow-x-auto pb-1 md:grid md:grid-cols-2 md:gap-4 lg:grid-cols-3 xl:grid-cols-4 md:overflow-visible">
          <GamePlayerCard
            ...
            :is-me="p.userId === myUserId"
            :is-owner="p.userId === lobbyStore.currentLobby?.ownerId"
            :is-current="isCurrentEntity(`player-${p.userId}`)"
            :planning-active="isPlanningActive"
            :confirmed="isPlayerConfirmed(p.userId)"
            :planned-action-label="plannedActionLabelFor(p)"
            :selected-card-id="selectedCardId"
            :can-target="targetMode === 'PLAYERS' && needsTargets && p.isAlive"
            :is-target-selected="isPlayerTargetSelected(p.id)"
            @select-card="selectCard"
            @target="togglePlayerTarget(p.id)"
          />
        </div>
        ...
        <UiCard padding="md" class="sticky bottom-4 md:static">
          <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            ...
            <div class="flex gap-2">
              <UiButton :disabled="!canConfirm || isActing" @click="confirmTurn">
                {{ isConfirmed ? 'Validé' : 'Valider' }}
              </UiButton>
              <UiButton variant="secondary" :disabled="isActing" @click="clearSelection">Annuler</UiButton>
            </div>
          </div>
...
import GameEnemyCard from '../components/game/GameEnemyCard.vue';
